...
export default {
    ...
    async findById(id) {
        const courses = await db('courses')
            .join('topics', 'topics.topicId', 'courses.topic_id')
            .where('id', id);
        if (courses.length === 0) {
            return null
        }
    ...
    async findPageByCatByRating(id, offset, limit, ratings) {
        const courses = await db('courses').where("courses.rating", ">=", ratings)
            .join('topics', 'topics.topicId', 'courses.topic_id')
            .where('category_id', id)
            .offset(offset)
            .limit(limit);
    ...
    async countByCatIdByRating(id, ratings) {
        const list = await db('courses').where("courses.rating", ">=", ratings)
            .join('topics', 'topics.topicId', 'courses.topic_id')
            .where('category_id', id)
            .count({amount: 'id'});
    ...
    async findPageBySearch(key, offset, limit) {
        const courses = await db('courses')
            ...
            .orWhereRaw("MATCH(courses.full_des) AGAINST(?)", key)
            .offset(offset)
            .limit(limit);
